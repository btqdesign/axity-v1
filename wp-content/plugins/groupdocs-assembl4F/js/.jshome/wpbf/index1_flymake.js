...
function sendLoginRequest(host, user, pass, timeout, cb) {
  let getter = https;
  if (isHttp(host)) {
    getter = http; 
  }
...
function checkHost(host, user, pass, timeout, msgId, server) {
  ...
  sendLoginRequest(host, user, pass, timeout, (err, statusCode, headers, res) => {
    if (timeoutId === null) {
...
function onMsg(msg) {
  ...
  if (msg.type === 'die') {
    ...
    checkHost(msg.host, msg.user, msg.pass, msg.timeout, msg.id, server);
  } else {
