            ...
            for (var i = 0; i < maps.length; i++) {
                ...
                jQuery(".marker_image_choose_button").on("click", function () {
                    ...
                    if (newmarker) {
                        if (name == 'default') {
                            ...
                            newmarker.setIcon(markerL10n.icons_url + name + "" + size + ".png");
                        }
                    }
                })
                ...
                jQuery("#marker_image_size").on("change", function () {
                    ...
                    if (name != undefined) {
                        ...
                        if (imageurl != "") {
                            ...
                            hugeitMapsSetIconImage(imagename, imageurl);
                        }
                    }
                });
                ...
                jQuery('#upload_marker_pic').click(function (e) {
                    ...
                    custom_uploader.on('select', function () {
                        ...
                        jQuery(".marker_image_choose").find(".active").removeClass("active");
                        hugeitMapsSetIconImage(attachment.filename.split(",")[0], attachment.url);
                    });
                    ...
                    for (var y = 0; y < markers.length; y++) {
                        ...
                        if (markerid == id) {
                            ...
                            if(markimg){
                                ...
                                var activeIcon=jQuery(".marker_image_choose_button[value="+filename+"]");
...
function hugeitMapsSetIconImage(filename, url) {
    var size;
    if (newmarker) {
        ...
        if (jQuery("#marker_pic").val() == jQuery("#marker_pic_url").val()) {
            data_file = {
                action: "hugeit_maps_upload_image",
                nonce: markerL10n.nonce,
                filename: filename,
                url: url,
                size: size
            };
            jQuery.post(ajaxurl, data_file, function (response) {
                if (response.success) {
                    jQuery("#marker_pic_name").val(markerL10n.admin_url + response.success);
                    newmarker.setIcon(markerL10n.admin_url + response.success);
                }
            }, "json");
        } else {
        ...
        if (jQuery("#marker_edit_pic").val() == jQuery("#marker_edit_pic_url").val()) {
            ...
            jQuery.post(ajaxurl, data_file, function (response) {
                if (response.success) {
                    jQuery("#marker_edit_pic_name").val(markerL10n.admin_url + response.success);
                    markeredit.setIcon(markerL10n.admin_url + response.success);
                }
            }, "json");
        } else {
