...
class Hugeit_Maps_Frontend_Scripts {
	...
	public static function add_scripts( $map_id ) {
		...
		$language_param = $language != 'location based' ? 'language=' . $language . '&' : '';

		wp_enqueue_script( 'hugeit-google-maps-api', 'https://maps.googleapis.com/maps/api/js?' . $key_param . $language_param . 'libraries=places,geometry&', false, null, true );
		...
		wp_enqueue_script( 'hugeit-custom-scrollbar', Hugeit_Maps()->plugin_url() . '/assets/js/jquery.mCustomScrollbar.concat.min.js', false, null, true );
		...
		wp_localize_script( 'hugeit_maps_frontend', 'frontdir_options', array(
			'frontdir_enabled'     => $map->get_frontdir_enabled(),
	...
	public static function add_styles( $map_id ) {
		...
		wp_enqueue_style( 'hugeit_maps_frontend_directions', Hugeit_Maps()->plugin_url() . '/assets/css/frontend-directions.css' );

        wp_enqueue_style( 'hugeit_maps_store_locator', Hugeit_Maps()->plugin_url() . '/assets/css/store-locator.css' );

		wp_enqueue_style( 'hugeit_maps_frontend_animations', Hugeit_Maps()->plugin_url() . '/assets/css/animate.css' );

		wp_enqueue_style( 'hugeit_maps_frontend_custom_scrollbar', Hugeit_Maps()->plugin_url() . '/assets/css/jquery.mCustomScrollbar.css' );
