...
class Hugeit_Maps_Ajax {
	...
	public static function init() {
		...
		add_action( 'wp_ajax_hugeit_maps_save_stylings', array( __CLASS__, 'save_stylings' ) );
		add_action( 'wp_ajax_hugeit_maps_save_layers', array( __CLASS__, 'save_layers' ) );
		...
		add_action( 'wp_ajax_hugeit_maps_save_frontdir', array( __CLASS__, 'save_frontdir' ) );
		...
		add_action( 'wp_ajax_hugeit_maps_edit_direction', array( __CLASS__, 'edit_direction' ) );
		add_action( 'wp_ajax_hugeit_maps_save_new_direction', array( __CLASS__, 'save_new_direction' ) );
	...
	public function edit_circle() {
		...
		try {
			$circle
				...
				->set_hover_fill_color( $_REQUEST['hover_circle_edit_fill_color'] )
				->set_hover_fill_opacity( $_REQUEST['hover_circle_edit_fill_opacity'] )
				->set_hover_line_color( $_REQUEST['hover_circle_edit_line_color'] )
				->set_hover_line_opacity( $_REQUEST['hover_circle_edit_line_opacity'] );
		} catch ( Exception $e ) {
			die( $e->getMessage() );
		}
	...
	public function save_circle() {
		...
		try {
			$circle
				...
				->set_hover_fill_color( $_REQUEST['hover_circle_fill_color'] )
				->set_hover_fill_opacity( $_REQUEST['hover_circle_fill_opacity'] )
				->set_hover_line_color( $_REQUEST['hover_circle_line_color'] )
				->set_hover_line_opacity( $_REQUEST['hover_circle_line_opacity'] );
		} catch ( Exception $e ) {
			die( $e->getMessage() );
		}
	...
	public static function edit_polyline() {
		...
		try {
			$polyline
				...
				->set_hover_line_color( $_REQUEST['hover_polyline_edit_line_color'] )
				->set_hover_line_opacity( $_REQUEST['hover_polyline_edit_line_opacity'] );
		} catch ( Exception $e ) {
			die( $e->getMessage() );
		}
	...
	public static function save_polyline() {
		...
		try {
			...
			$polyline
				...
				->set_hover_line_color( $_REQUEST['hover_polyline_line_color'] )
				->set_hover_line_opacity( $_REQUEST['hover_polyline_line_opacity'] );
	...
	public static function edit_polygon() {
		...
		try {
			$polygon
				...
				->set_url( $_REQUEST['polygone_edit_url'] )
				...
				->set_hover_fill_opacity( $_REQUEST['hover_polygone_edit_fill_opacity'] )
				->set_hover_fill_color( $_REQUEST['hover_polygone_edit_fill_color'] )
				->set_hover_line_opacity( $_REQUEST['hover_polygone_edit_line_opacity'] )
				->set_hover_line_color( $_REQUEST['hover_polygone_edit_line_color'] )
	...
	public static function save_polygon() {
		...
		try {
			$polygon
				...
				->set_hover_fill_opacity( $_REQUEST['hover_polygon_fill_opacity'] )
				->set_hover_fill_color( $_REQUEST['hover_polygon_fill_color'] )
				->set_hover_line_opacity( $_REQUEST['hover_polygon_line_opacity'] )
				->set_hover_line_color( $_REQUEST['hover_polygon_line_color'] );
		} catch ( Exception $e ) {
			die( $e->getMessage() );
		}
	...
	public static function edit_marker() {
		...
		try {
			$marker
				...
				->set_img( $_REQUEST['marker_edit_image'] )
				->set_size( $_REQUEST['marker_edit_image_size'] );
		} catch ( Exception $e ) {
			die( $e->getMessage() );
		}
	...
	public static function save_marker() {
		...
		try {
			$marker
				...
				->set_img( $_REQUEST['marker_image'] )
				->set_size( $_REQUEST['marker_image_size'] );
		} catch ( Exception $e ) {
			die( $e->getMessage() );
		}
	...
	public static function save_map() {
		...
		try {
			$map->set_name( $_REQUEST['map_name'] )
				->set_info_type( $_REQUEST['map_infowindow_type'] )
				...
				->set_wheel_scroll( $_REQUEST['wheel_scroll'] )
				->set_draggable( $_REQUEST['draggable'] )
				->set_language( $_REQUEST['map_language'] )
				->set_border_radius( $_REQUEST['map_border_radius'] )
				->set_animation( $_REQUEST['map_animation'] )
		...
		if ( $saved ) {
		   ...
		   global $wpdb;
           wp_die( 'something went wrong 10' );
		}
	}
	 ...
	 * Save layers data
	 */
	public static function save_layers() {
		if ( ! isset( $_REQUEST['nonce'] ) || ! wp_verify_nonce( $_REQUEST['nonce'], 'hugeit-maps-map-save' ) ) {
			wp_die( __( 'Security check failed', 'hugeit_maps' ) );
		}
		...
		$map
			->set_traffic_layer( $_REQUEST['traffic'] )
			->set_bike_layer( $_REQUEST['bike'] )
			->set_transit_layer( $_REQUEST['transit'] );
    ...
    /**
     * Save Front End Directions data
     */
    public static function save_frontdir() {
        if ( ! isset( $_REQUEST['nonce'] ) || ! wp_verify_nonce( $_REQUEST['nonce'], 'hugeit-maps-map-save' ) ) {
            wp_die( __( 'Security check failed', 'hugeit_maps' ) );
        }
        ...
        $map
            ->set_frontdir_enabled( $_REQUEST['frontdir_enabled'] )
            ->set_frontdir_align( $_REQUEST['frontdir_align'] )
            ->set_dir_window_width( $_REQUEST['dir_window_width'] );
	 ...
	 * Save direction info
	 */
	public static function save_new_direction() {
		if ( ! isset( $_REQUEST['nonce'] ) || ! wp_verify_nonce( $_REQUEST['nonce'], 'hugeit-maps-direction-save' ) ) {
			wp_die( __( 'Security check failed', 'hugeit_maps' ) );
		}
		...
		$direction = new Hugeit_Maps_Direction();

		$direction
			->set_map_id( $map_id )
			->set_name( $_REQUEST['name'] )
			->set_start_lat( $_REQUEST['startLat'] )
			->set_start_lng( $_REQUEST['startLng'] )
			->set_end_lat( $_REQUEST['endLat'] )
			->set_end_lng( $_REQUEST['endLng'] )
			->set_travel_mode( $_REQUEST['travelMode'] )
			->set_show_steps( $_REQUEST['showSteps'] )
			->set_line_opacity( $_REQUEST['lineOpacity'] )
			->set_line_color( $_REQUEST['lineColor'] )
			->set_line_width( $_REQUEST['lineWidth'] );

		$saved = $direction->save();
	...
	public static function edit_direction() {
		...
		$direction = new Hugeit_Maps_Direction( absint( $id ) );
...
/* register_shutdown_function(function () {
		...
		foreach ( $markers as $marker ) {
			$response['markers'][] = array(
				...
				'lat'         => $marker->get_lat(),
				'lng'         => $marker->get_lng(),
				'description' => (isset($_REQUEST['processShortcode']))?do_shortcode($marker->get_description()):$marker->get_description(),
				'img'         => $marker->get_img(),
			);
		}
