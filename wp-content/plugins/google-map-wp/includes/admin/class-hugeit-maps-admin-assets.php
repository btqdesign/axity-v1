...
class Hugeit_Maps_Admin_Assets {
	...
	public static function admin_styles( $hook ){
        ...
        if( isset($_GET['task']) && $_GET['task'] == 'edit_map' ){
            wp_enqueue_style( 'front-directions-css', Hugeit_Maps()->plugin_url().'/assets/css/frontend-directions.css' );
        }
	...
	public static function admin_scripts( $hook ){
		...
		if( $hook === Hugeit_Maps()->admin->pages['main_page']  ){
            ...
            if( isset($_GET['task']) && $_GET['task'] == 'edit_map' ){
                ...
                wp_enqueue_script( "google-maps-api", 'https://maps.googleapis.com/maps/api/js?'.$key_param.'libraries=places,geometry' );
                ...
                wp_enqueue_script( 'hugeit-maps-admin-directions', Hugeit_Maps()->plugin_url().'/assets/js/admin/directions.js', array( 'jquery' ), false, true );
                ...
                wp_enqueue_script( 'hugeit-maps-admin-directions-save', Hugeit_Maps()->plugin_url().'/assets/js/admin/directions-save.js', array( 'jquery' ), false, true );
	...
	private static function localize_scripts(){
        ...
        wp_localize_script( 'hugeit-maps-admin-directions', 'directionL10n', array(
            'map'=>$localized_map,
            'startPointTitle' => __( 'Start Point', 'hugeit_maps' ),
            'invalidDirectionPoints' => __( 'Could not construct a route with the given options', 'hugeit_maps' )
        ) );
        ...
        wp_localize_script( 'hugeit-maps-admin-directions-save', 'directionSaveL10n',array(
            'nonce'=>wp_create_nonce( 'hugeit-maps-direction-save' )
        ) );
